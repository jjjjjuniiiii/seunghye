...
<div class="table-wrap">
	<table id="table">			
		...
		<tbody id="addCell">			
			<c:forEach var="pos" items="${posList4}">
				<tr>		
					<td style="display: none">${pos.sequence}</td>
					<td nowrap>${pos.large_category}</td>
					<td nowrap>${pos.small_category}</td>
					<td nowrap>${pos.system}</td>
					<td nowrap>${pos.using}</td>
					<td nowrap>${pos.host_name}</td>
					<td nowrap>${pos.manufacturer}</td>
					<td nowrap>${pos.model_name}</td>
					<td nowrap>${pos.specification}</td>
					<td nowrap>${pos.purchase_num}</td>
					<td nowrap>${pos.install_name}</td>
					<td nowrap>${pos.install_num}</td>
					<td nowrap>${pos.start_date}</td>
					<td nowrap>${pos.deterioration}</td>
					<td nowrap>${pos.start_money}</td>
					<td nowrap>${pos.place}</td>
					<td nowrap>${pos.location}</td>
					<td nowrap>${pos.status_progress}</td>
					<td nowrap>${pos.contract_term}</td>
					<td nowrap>${pos.month_money}</td>
					<td nowrap>${pos.status_use}</td>
					<td nowrap>${pos.disuse_or_recycling}</td>
					<td nowrap>${pos.reason_disuse}</td>
					<td nowrap>${pos.note}</td>
					<td nowrap>${pos.unification}</td>
					<td nowrap>${pos.EOS_end_date}</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
...
<div class="w3-display-container w3-pale-red w3-padding-16">
	<div class="w3-container w3-right">
		<button type="button" onclick="cancle_divide();" class="w3-button w3-black w3-margin-right w3-margin-bottom" >분리취소</button>
...
$("#table tr").click(function(){ 		
	...
	$("#input17").val(status_use);
	$("#input18").val(disuse_or_recycling);
	$("#input19").val(reason_disuse);
	$("#input20").val(note);
	$("#input21").val(unification);
	$("#input22").val(EOS_end_date);	
});

//분리취소 버튼 클릭시 POS&PDA(구)페이지에 조회
function cancle_divide(){
	if(confirm("분리를 취소 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/posDisuse4";
		f.submit();
	}
	else{
		return false;
...
function update(){
	if(confirm("수정 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/posUpdate4";
		f.submit();
	}
	else{
		return false;
...
function deleteData(){
	if(confirm("삭제 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/posDelete4";
		f.submit();
	}
	else{
		return false;
...
function insert(){
	...
	if(f.box1.value=="" || f.box2.value=="" || f.box3.value=="" || f.input17.value=="" || f.input21.value==""){
		...
		if(confirm("추가 하시겠습니까?")==true){
			f.action = "${ pageContext.servletContext.contextPath }/page/user/posInsert4";
			f.submit();
		}
		else{
			return false;
