...
<div class="table-wrap">
	<table id="table">			
		...
		<tbody id="addCell">			
			<c:forEach var="giftcard" items="${giftcardList1}">
				<tr>		
					<td style="display: none">${giftcard.sequence}</td>
					<td nowrap>${giftcard.large_category}</td>
					<td nowrap>${giftcard.small_category}</td>
					<td nowrap>${giftcard.system}</td>
					<td nowrap>${giftcard.using}</td>
					<td nowrap>${giftcard.host_name}</td>
					<td nowrap>${giftcard.manufacturer}</td>
					<td nowrap>${giftcard.model_name}</td>
					<td nowrap>${giftcard.specification}</td>
					<td nowrap>${giftcard.purchase_num}</td>
					<td nowrap>${giftcard.install_name}</td>
					<td nowrap>${giftcard.install_num}</td>
					<td nowrap>${giftcard.start_date}</td>
					<td nowrap>${giftcard.deterioration}</td>
					<td nowrap>${giftcard.start_money}</td>
					<td nowrap>${giftcard.place}</td>
					<td nowrap>${giftcard.location}</td>
					<td nowrap>${giftcard.status_progress}</td>
					<td nowrap>${giftcard.contract_term}</td>
					<td nowrap>${giftcard.month_money}</td>
					<td nowrap>${giftcard.status_use}</td>
					<td nowrap>${giftcard.disuse_or_recycling}</td>
					<td nowrap>${giftcard.reason_disuse}</td>
					<td nowrap>${giftcard.note}</td>
					<td nowrap>${giftcard.unification}</td>
					<td nowrap>${giftcard.EOS_end_date}</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
...
function disuse(){
	if(confirm("폐기 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/giftcardDisuse1";
		f.submit();
	}
	else{
		return false;
...
function update(){
	if(confirm("수정 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/giftcardUpdate1";
		f.submit();
	}
	else{
		return false;		
...
function deleteData(){
	if(confirm("삭제 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/giftcardDelete1";
		f.submit();
	}
	else{
		return false;
...
function insert(){
	...
	if(f.box1.value=="" || f.box2.value=="" || f.box3.value=="" || f.input17.value=="" || f.input21.value==""){
		...
		if(confirm("추가 하시겠습니까?")==true){
			f.action = "${ pageContext.servletContext.contextPath }/page/user/giftcardInsert1";
			f.submit();
		}
		else{
			return false;
