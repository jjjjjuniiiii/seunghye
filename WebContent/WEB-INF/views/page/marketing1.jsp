...
<div class="table-wrap">
	<table id="table">			
		...
		<tbody id="addCell">			
			<c:forEach var="marketing" items="${marketingList1}">
				<tr>		
					<td style="display: none">${marketing.sequence}</td>
					<td nowrap>${marketing.large_category}</td>
					<td nowrap>${marketing.small_category}</td>
					<td nowrap>${marketing.system}</td>
					<td nowrap>${marketing.using}</td>
					<td nowrap>${marketing.host_name}</td>
					<td nowrap>${marketing.manufacturer}</td>
					<td nowrap>${marketing.model_name}</td>
					<td nowrap>${marketing.specification}</td>
					<td nowrap>${marketing.purchase_num}</td>
					<td nowrap>${marketing.install_name}</td>
					<td nowrap>${marketing.install_num}</td>
					<td nowrap>${marketing.start_date}</td>
					<td nowrap>${marketing.deterioration}</td>
					<td nowrap>${marketing.start_money}</td>
					<td nowrap>${marketing.place}</td>
					<td nowrap>${marketing.location}</td>
					<td nowrap>${marketing.status_progress}</td>
					<td nowrap>${marketing.contract_term}</td>
					<td nowrap>${marketing.month_money}</td>
					<td nowrap>${marketing.status_use}</td>
					<td nowrap>${marketing.disuse_or_recycling}</td>
					<td nowrap>${marketing.reason_disuse}</td>
					<td nowrap>${marketing.note}</td>
					<td nowrap>${marketing.unification}</td>
					<td nowrap>${marketing.EOS_end_date}</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
...
<div class="w3-display-container w3-pale-red w3-padding-16">
	<div class="w3-container w3-right">
		<button type="button" onclick="disuse();" class="w3-button w3-black w3-margin-right w3-margin-bottom" >폐기</button>
...
//폐기 버튼 클릭시 폐기처리
function disuse(){
	if(confirm("폐기 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/marketingDisuse1";
		f.submit();
	}
	else{
		return false;
...
function update(){
	if(confirm("수정 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/marketingUpdate1";
		f.submit();
	}
	else{
		return false;		
...
function deleteData(){
	if(confirm("삭제 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/marketingDelete1";
		f.submit();
	}
	else{
		return false;
...
$('#resetButton').click(function(){
	...
	$("#input15").val("");
	$("#input16").val("");
	$("#input17 option:eq(0)").prop("selected", true);
	$("#input18").val("");
	$("#input19").val("");
	$("#input20").val("");
	$("#input21 option:eq(0)").prop("selected", true);
	$("#input22").val("");
	$("#input23").val("");
	$("#input24").val("");
});
...
function insert(){
	...
	if(f.box1.value=="" || f.box2.value=="" || f.box3.value=="" || f.input17.value=="" || f.input21.value==""){
		...
		if(confirm("추가 하시겠습니까?")==true){
			f.action = "${ pageContext.servletContext.contextPath }/page/user/marketingInsert1";
			f.submit();
		}
		else{
			return false;
