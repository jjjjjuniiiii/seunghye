...
<div class="table-wrap">
	<table id="table">			
		...
		<tbody id="addCell">			
			<c:forEach var="md" items="${mdList1}">
				<tr>		
					<td style="display: none">${md.sequence}</td>
					<td nowrap>${md.large_category}</td>
					<td nowrap>${md.small_category}</td>
					<td nowrap>${md.system}</td>
					<td nowrap>${md.using}</td>
					<td nowrap>${md.host_name}</td>
					<td nowrap>${md.manufacturer}</td>
					<td nowrap>${md.model_name}</td>
					<td nowrap>${md.specification}</td>
					<td nowrap>${md.purchase_num}</td>
					<td nowrap>${md.install_name}</td>
					<td nowrap>${md.install_num}</td>
					<td nowrap>${md.start_date}</td>
					<td nowrap>${md.deterioration}</td>
					<td nowrap>${md.start_money}</td>
					<td nowrap>${md.place}</td>
					<td nowrap>${md.location}</td>
					<td nowrap>${md.status_progress}</td>
					<td nowrap>${md.contract_term}</td>
					<td nowrap>${md.month_money}</td>
					<td nowrap>${md.status_use}</td>
					<td nowrap>${md.disuse_or_recycling}</td>
					<td nowrap>${md.reason_disuse}</td>
					<td nowrap>${md.note}</td>
					<td nowrap>${md.unification}</td>
					<td nowrap>${md.EOS_end_date}</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
...
function disuse(){
	if(confirm("폐기 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/mdDisuse1";
		f.submit();
	}
	else{
		return false;
...
function update(){
	if(confirm("수정 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/mdUpdate1";
		f.submit();
	}
	else{
		return false;		
...
function deleteData(){
	if(confirm("삭제 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/mdDelete1";
		f.submit();
	}
	else{
		return false;
...
function insert(){
	...
	if(f.box1.value=="" || f.box2.value=="" || f.box3.value=="" || f.input17.value=="" || f.input21.value==""){
		...
		if(confirm("추가 하시겠습니까?")==true){
			f.action = "${ pageContext.servletContext.contextPath }/page/user/mdInsert1";
			f.submit();
		}
		else{
			return false;
