...
<div class="table-wrap">
	<table id="table">			
		...
		<tbody id="addCell">			
			<c:forEach var="infra" items="${infraList3}">
				<tr>		
...
function disuse(){
	if(confirm("폐기 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/infraDisuse3";
		f.submit();
	}
	else{
		return false;
...
function update(){
	if(confirm("수정 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/infraUpdate3";
		f.submit();
	}
	else{
		return false;		
...
function deleteData(){
	if(confirm("삭제 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/infraDelete3";
		f.submit();
	}
	else{
		return false;
...
function insert(){
	...
	if(f.box1.value=="" || f.box2.value=="" || f.box3.value=="" || f.input17.value=="" || f.input21.value==""){
		...
		if(confirm("추가 하시겠습니까?")==true){
			f.action = "${ pageContext.servletContext.contextPath }/page/user//page/user/infraInsert3";
			f.submit();
		}
		else{
			return false;
