...
<div class="table-wrap">
	<table id="table">			
		...
		<tbody id="addCell">			
			<c:forEach var="support" items="${supportList1}">
				<tr>		
					<td style="display: none">${support.sequence}</td>
					<td nowrap>${support.large_category}</td>
					<td nowrap>${support.small_category}</td>
					<td nowrap>${support.system}</td>
					<td nowrap>${support.using}</td>
					<td nowrap>${support.host_name}</td>
					<td nowrap>${support.manufacturer}</td>
					<td nowrap>${support.model_name}</td>
					<td nowrap>${support.specification}</td>
					<td nowrap>${support.purchase_num}</td>
					<td nowrap>${support.install_name}</td>
					<td nowrap>${support.install_num}</td>
					<td nowrap>${support.start_date}</td>
					<td nowrap>${support.deterioration}</td>
					<td nowrap>${support.start_money}</td>
					<td nowrap>${support.place}</td>
					<td nowrap>${support.location}</td>
					<td nowrap>${support.status_progress}</td>
					<td nowrap>${support.contract_term}</td>
					<td nowrap>${support.month_money}</td>
					<td nowrap>${support.status_use}</td>
					<td nowrap>${support.disuse_or_recycling}</td>
					<td nowrap>${support.reason_disuse}</td>
					<td nowrap>${support.note}</td>
					<td nowrap>${support.unification}</td>
					<td nowrap>${support.EOS_end_date}</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
...
function disuse(){
	if(confirm("폐기 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/supportDisuse1";
		f.submit();
	}
	else{
		return false;
...
function update(){
	if(confirm("수정 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/supportUpdate1";
		f.submit();
	}
	else{
		return false;		
...
function deleteData(){
	if(confirm("삭제 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/supportDelete1";
		f.submit();
	}
	else{
		return false;
...
function insert(){
	...
	if(f.box1.value=="" || f.box2.value=="" || f.box3.value=="" || f.input17.value=="" || f.input21.value==""){
		...
		if(confirm("추가 하시겠습니까?")==true){
			f.action = "${ pageContext.servletContext.contextPath }/page/user/supportInsert1";
			f.submit();
		}
		else{
			return false;
