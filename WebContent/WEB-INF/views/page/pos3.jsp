...
<div class="table-wrap">
	<table id="table">			
		...
		<tbody id="addCell">			
			<c:forEach var="pos" items="${posList3}">
				<tr>		
...
<div class="w3-display-container w3-pale-red w3-padding-16">
	<div class="w3-container w3-right">
		<button type="button" onclick="divide();" class="w3-button w3-black w3-margin-right w3-margin-bottom" >분리</button>
...
//분리 버튼 클릭시 POS&PDA(신)페이지에 조회
function divide(){
	if(confirm("분리 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/posDisuse3";
		f.submit();
	}
	else{
		return false;
...
function update(){
	if(confirm("수정 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/posUpdate3";
		f.submit();
	}
	else{
		return false;
...
function deleteData(){
	if(confirm("삭제 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/posDelete3";
		f.submit();
	}
	else{
		return false;
...
function insert(){
	...
	if(f.box1.value=="" || f.box2.value=="" || f.box3.value=="" || f.input17.value=="" || f.input21.value==""){
		...
		if(confirm("추가 하시겠습니까?")==true){
			f.action = "${ pageContext.servletContext.contextPath }/page/user/posInsert3";
			f.submit();
		}
		else{
			return false;
