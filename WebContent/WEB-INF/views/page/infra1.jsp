...
<div class="table-wrap">
	<table id="table">			
		...
		<tbody id="addCell">			
			<c:forEach var="infra" items="${infraList1}">
				<tr>		
					<td style="display: none">${infra.sequence}</td>
					<td nowrap>${infra.large_category}</td>
					<td nowrap>${infra.small_category}</td>
					<td nowrap>${infra.system}</td>
					<td nowrap>${infra.using}</td>
					<td nowrap>${infra.host_name}</td>
					<td nowrap>${infra.manufacturer}</td>
					<td nowrap>${infra.model_name}</td>
					<td nowrap>${infra.specification}</td>
					<td nowrap>${infra.purchase_num}</td>
					<td nowrap>${infra.install_name}</td>
					<td nowrap>${infra.install_num}</td>
					<td nowrap>${infra.start_date}</td>
					<td nowrap>${infra.deterioration}</td>
					<td nowrap>${infra.start_money}</td>
					<td nowrap>${infra.place}</td>
					<td nowrap>${infra.location}</td>
					<td nowrap>${infra.status_progress}</td>
					<td nowrap>${infra.contract_term}</td>
					<td nowrap>${infra.month_money}</td>
					<td nowrap>${infra.status_use}</td>
					<td nowrap>${infra.disuse_or_recycling}</td>
					<td nowrap>${infra.reason_disuse}</td>
					<td nowrap>${infra.note}</td>
					<td nowrap>${infra.unification}</td>
					<td nowrap>${infra.EOS_end_date}</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
...
function disuse(){
	if(confirm("폐기 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/infraDisuse1";
		f.submit();
	}
	else{
		return false;
...
function update(){
	if(confirm("수정 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user//page/user/infraUpdate1";
		f.submit();
	}
	else{
		return false;		
...
function deleteData(){
	if(confirm("삭제 하시겠습니까?")==true){
		var f = document.form;
		f.action = "${ pageContext.servletContext.contextPath }/page/user/infraDelete1";
		f.submit();
	}
	else{
		return false;
...
function insert(){
	...
	if(f.box1.value=="" || f.box2.value=="" || f.box3.value=="" || f.input17.value=="" || f.input21.value==""){
		...
		if(confirm("추가 하시겠습니까?")==true){
			f.action = "${ pageContext.servletContext.contextPath }/page/user/infraInsert1";
			f.submit();
		}
		else{
			return false;
